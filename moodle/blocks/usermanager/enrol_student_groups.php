...
//$SESSION->{$courseid}->disciplins = $DB->get_records_sql($sql);
...
if ($mform->is_cancelled()) {
    $url = new moodle_url('/course/view.php', array('courseid' => $courseid));
    redirect($url);
        ...
        foreach ($groups_of_users as $group_id => $group_of_user) {
            ...
            if (in_array($check, $groups_selected, $strict = true)) {
                ...
                if (!$moodle_group_info) {
                    $moodle_group_name = '';
                    $faculty = explode(' ', $disciplins_with_number->{$disciplin_id}->faculty);
                    ...
                    $moodle_group_name .= $disciplins_with_number->{$disciplin_id}->year.'г. ';
    ...
    //$SESSION->{$courseid}->groups_of_users = $groups_of_users_new; //for update enrolled information in form
...
$mform->display();
//echo var_dump($group_of_user);
//echo '</br>';
//echo var_dump($checkk);
echo $OUTPUT->footer();
